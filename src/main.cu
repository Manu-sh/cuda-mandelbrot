...
__device__ Pixel calc_mandelbrot(uint16_t ix, uint16_t iy) {
    ...
    // that took me a lot debugging to figure why the image generated with (uint8_t)(z.real() * 1) was so different 
    // than another generated by the same code but running on cpu instead of gpu
    // (uint8_t)(int)(z.real() * 1) could lead signed integer overflow
...
__global__ void kernel(Pixel *const v, uint32_t len) {
    ...
    // https://cuda-tutorial.readthedocs.io/en/latest/tutorials/tutorial02/#exercise-1-parallelizing-vector-addition-using-multithread
    #pragma unroll
