...
struct PGM : StandardMatrix1D<GrayScale> {
    ...
    constexpr FORCED(inline) const PGM & write_file_content(const char *const file_name) const {
        if constexpr(format == pnm::Format::PGM2)
            return write_file_content_ppm2(file_name), *this;

        return write_file_content_ppm5(file_name), *this;
    }

    private:
        const PGM & write_file_content_ppm2(const char *const file_name) const;
        const PGM & write_file_content_ppm5(const char *const file_name) const;
