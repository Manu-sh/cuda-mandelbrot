...
namespace pnm {
    ...
    struct ChannelDepth<pnm::BIT_8> {
        struct max_value {
            ...
            constexpr operator uint8_t() const noexcept { return 0xff; }
        };
    };
    ...
    struct AbstractHeader {
        ...
        public:
            operator std::string() {
                using depth_t = typename ChannelDepth<BIT_DEPTH>::max_value;
                std::ostringstream ss;
                ss << this->m_type << '\n'                                                           // ppm type
                   << std::to_string(this->m_width) << ' ' << std::to_string(this->m_height) << '\n' // width x height
                   << std::string_view(depth_t{}) << '\n';                                           // MAX_VALUE, end of header
